...
const recentVKeys = [
  ...
  { id: "VK-4567-8901", status: "unused", usedAt: null },
  { id: "VK-5678-9012", status: "used", usedAt: "2023-10-05" },
]
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="rounded-md border">
                <div className="grid grid-cols-3 gap-4 bg-muted/50 p-3 text-sm font-medium">
                  <div>Key</div>
                  <div>Status</div>
                  <div>Used At</div>
                </div>
                <div className="divide-y">
                  {recentVKeys.slice(0, 3).map((vkey) => (
                    <div key={vkey.id} className="grid grid-cols-3 gap-4 p-3 text-sm">
                      <div className="font-medium text-[#1e3a8a]">{vkey.id}</div>
                      <div>
                        <StatusBadge status={vkey.status as "used" | "unused"} />
                      </div>
                      <div className="text-muted-foreground">{vkey.usedAt || "Not used yet"}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-center">
              <div className="relative h-32 w-32">
                <Image src="/images/world-records-logo.png" alt="World Records Logo" fill className="object-contain" />
              </div>
            </div>
